# This migration comes from spree (originally 20210527094055)
    ...
    if table_exists?(:spree_products_stores)
      ...
      if product_ids.any? && Spree::StoreProduct.respond_to?(:insert_all)
        stores.find_each do |store|
          records = product_ids.map do |product_id|
            { product_id: product_id, store_id: store.id, created_at: Time.zone.now, updated_at: Time.zone.now }
          end
